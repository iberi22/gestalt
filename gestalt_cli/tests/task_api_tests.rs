...
"#;
    ...
    let temp_db_path =
        std::env::temp_dir().join(format!("gestalt_test_task_api_{}.db", uuid::Uuid::new_v4()));
    ...
    let body_bytes_no_match = axum::body::to_bytes(response_no_match.into_body(), usize::MAX)
        .await
        .unwrap();
    let res_json_no_match: serde_json::Value =
        serde_json::from_slice(&body_bytes_no_match).unwrap();

    assert_eq!(
        res_json_no_match.get("status").unwrap().as_str().unwrap(),
        "error"
    );
    assert!(res_json_no_match
        .get("message")
        .unwrap()
        .as_str()
        .unwrap()
        .contains("No matching agents found"));

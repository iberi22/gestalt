#!/usr/bin/env bash
# =============================================================================
# verify-pipeline.sh — GitCore local CI / reality pipeline (generic, portable)
...
#   2. SRS REQ-IDs exist in docs/SRS/REQUIREMENTS.md
#   3. User stories exist in docs/SRS/USER-STORIES.md
#   4. Tests referenced exist in source (best-effort final-symbol grep)
#   5. Code compiles / builds             [--check]
#   6. Tests run green                    [--test]
#
# Stack auto-detection (verified 2026-08-04 on shelf/worldexams/gara-g/xavier):
#   anchor (Anchor.toml + programs/Cargo.toml) | rust | pnpm | npm
#   Override with VERIFY_STACK=rust|anchor|pnpm|npm.
#   Anchor: Cargo.toml lives in programs/, NOT root — check Anchor.toml FIRST.
#   npm: canonical suite may be `npm run validate` (worldexams 1328 bundles).
#
# Usage:
...
if [ "$FEATURES_PRESENT" -eq 1 ]; then
  while IFS='|' read -r fid pct reqs us impl tests; do
    ...
    # 4. tests referenced (best-effort: final symbol name, first 3 tests)
    if [ -n "$tests" ]; then
      n=0

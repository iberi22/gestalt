    ...
    match args.command {
        ...
        Commands::Tools => {
            let tools: Vec<serde_json::Value> =
                reqwest::blocking::get("http://127.0.0.1:3000/tools")?.json()?;
        ...
        Commands::Analyze { path } => {
                ...
                .and_then(|c| c.get("text"))
            {
                if let Ok(a) = serde_json::from_str::<serde_json::Value>(c.as_str().unwrap_or("{}"))
                {
                    println!(
                        "📊 {} files",
                        a.get("total_files").and_then(|v| v.as_u64()).unwrap_or(0)
                    );
                }
            }
        }
        ...
        Commands::Search { pattern, path } => {
            ...
            {
                if let Ok(r) =
                    serde_json::from_str::<Vec<serde_json::Value>>(c.as_str().unwrap_or("[]"))
                {
        ...
        Commands::SysInfo => {
            ...
            {
                if let Ok(info) =
                    serde_json::from_str::<serde_json::Value>(c.as_str().unwrap_or("{}"))
                {
                    println!(
                        "💻 System: {} {}",

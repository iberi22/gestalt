...
impl VirtualFileSystem for OverlayFs {
    ...
    async fn exists(&self, path: &Path) -> Result<bool> {
        let state = self.state.lock().await;
        if state.text_files.contains_key(path) || state.binary_files.contains_key(path) || state.dirs.contains(path) {
            return Ok(true);
        }
        drop(state);
...
#[cfg(test)]
mod tests {
    use super::{FileEventType, FileWatcher, LockStatus, OverlayFs, PendingChange, VirtualFileSystem, RealFileSystem};
    use anyhow::Result;
    ...
    async fn test_minimal_vfs_methods() -> Result<()> {
        ...
        real_vfs.write(&file, b"hello minimal".to_vec(), "owner").await?;
        ...
        overlay_vfs.write(&overlay_file, b"hello overlay minimal".to_vec(), "owner").await?;
